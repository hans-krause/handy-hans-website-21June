...
export function createStripeClient(env: StripeEnv): Stripe {
  ...
  return new Stripe(connectionApiKey, {
    ...
    httpClient: Stripe.createFetchHttpClient((url: string | URL, init?: RequestInit) => {
      ...
      // The Stripe SDK adds `Authorization: Bearer <key>`. The connector
      // gateway rejects requests that carry both `Authorization` and
      // `Lovable-API-Key`, so strip Authorization and forward only the
      // gateway's auth scheme.
